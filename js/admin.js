...
function loadAdminSettings(){const saved=localStorage.getItem(ADMIN_STORE);
                             if(!saved)return;
                             try{const p=JSON.parse(saved);
                                 adminSettings={...adminSettings,...p,weekdayShiftPct:{...adminSettings.weekdayShiftPct,...(p.weekdayShiftPct||{})},saturdayShiftPct:{...adminSettings.saturdayShiftPct,...(p.saturdayShiftPct||{})}}}catch{}}
function saveAdminSettings(){localStorage.setItem(ADMIN_STORE,JSON.stringify(adminSettings))}

function initAdmin(){loadAdminSettings();
                     const toggle=document.getElementById('adminToggle');
                     const tabBtn=document.getElementById('adminTabBtn');
                     if(!toggle||!tabBtn)return;
                     toggle.onchange=()=>{if(toggle.checked){const pwd=prompt('Admin wachtwoord:');
                              if(pwd===ADMIN_PASSWORD){tabBtn.style.display='inline-block';
                              bindAdminInputs()}else{alert('Fout wachtwoord');
                              toggle.checked=false;
                              tabBtn.style.display='none'}}else{tabBtn.style.display='none'}}}

function setAdminValue(id,value){const el=document.getElementById(id);

                                 if(el)el.value=String(value).replace('.',',')}
function bindAdminNumber(id,getter,setter){const el=document.getElementById(id);
                                  if(!el)return;setAdminValue(id,getter());
                                           el.oninput=e=>{setter(parseNum(e.target.value));
                                                          saveAdminSettings();
                                                          triggerRecalc()}}
